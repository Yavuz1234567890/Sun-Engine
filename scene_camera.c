...
static void Tick()
{	
	camFront[0] = cos(glm_rad(yaw)) * cos(glm_radians(pitch));
	camFront[1] = sin(glm_rad(pitch));
	camFront[2] = sin(glm_rad(yaw)) * cos(glm_radians(pitch));
	...
	glm_perspective(glm_radians(fov), Width / Height, 0.1f, 500, proj);
